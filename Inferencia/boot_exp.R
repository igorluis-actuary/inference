#General Function
bootstrap_exp <- function(nobs = c(10, 30, 50), B = c(50, 100, 500), beta = c(1, 0.5), seed = 1987, 
      ...
      for(k in 1:length(B)){
        ...
        for(l in 1:B[k]){
          if(type == 'non-parametric'){
            ...
            est_x <- fitdistr(x, dexp, list(rate = 0.1), lower = c(0.001))
            resample_x <- rexp(length(x), rate = est_x$estimate)
          }
  ...
  if(export){
    name_file_export <- paste0('Simulation results exponential', ' - ', type, '.txt')
...
#set.seed(1987)
res <- bootstrap_exp(nobs = c(10, 30, 50), B = c(50, 100, 500), beta = c(0.5,1.0), seed = 1987, 
...
hist(x, prob = T,  main = paste0('N = 10 | Beta = ', beta[1]), col = "gray", xlab = '')
...
legend('topright', 1, cex= 1.5, lty = c(1:4), col= c(1:4),
...
x <- rexp(n = 30, rate = 1)
x2 <- seq(min(x), max(x), length = length(x))
hist(x, prob = T,  main = paste0('N = 30 | Beta = ', beta[1]), col = "gray", xlab = '')
...
legend('topright', cex= 1.5, lty = c(1:4), col= c(1:4),
       legend=c("EMV", "EMV B = 100", "EMV B = 300", "EMV B = 500"),
       box.lty=0)
...
x <- rexp(n = 50, rate = 1)
x2 <- seq(min(x), max(x), length = length(x))
hist(x, prob = T,  main = paste0('N = 50 | Beta = ', beta[1]), col = "gray", xlab = '')
...
x2 <- seq(min(x), max(x), length = length(x))
hist(x, prob = T,  main = paste0('N = 10 | Beta = ', beta[2]), col = "gray", xlab = '')
...
x2 <- seq(min(x), max(x), length = length(x))
hist(x, prob = T,  main = paste0('N = 30 | Beta = ', beta[2]), col = "gray", xlab = '')
...
x2 <- seq(min(x), max(x), length = length(x))
hist(x, prob = T,  main = paste0('N = 50 | Beta = ', beta[2]), col = "gray", xlab = '')
